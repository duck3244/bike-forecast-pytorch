...
class BikeForecasterTrainer:
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs=100, batch_size=64, lr=0.001,
              num_workers=2, use_amp=True, on_epoch_end=None, should_stop=None):
        """Train the model.

        ``on_epoch_end(event_dict)`` is invoked after every completed epoch with
        ``{epoch, train_loss, val_loss, best_val_loss, is_best}``. It may be
        called from any thread — the callback must be thread-safe.

        ``should_stop()`` is polled at the start of each epoch. Returning True
        stops training gracefully between epochs (the current in-flight epoch
        is not interrupted). Returns the number of epochs actually completed.
        """
        ...
        amp_enabled = bool(use_amp and cuda)
        scaler = torch.amp.GradScaler('cuda', enabled=amp_enabled)

        best_val_loss = float('inf')
        # Keep best weights in memory — avoids clobbering files across concurrent runs.
        ...
        for epoch in range(epochs):
            if should_stop is not None and should_stop():
                cancelled = True
                print(f'Training cancelled before epoch {epoch}')
                break
            # Training
            self.model.train()
            train_loss = 0.0
            ...
            for batch_X, batch_y in train_loader:
                batch_X = batch_X.to(self.device, non_blocking=cuda)
                batch_y = batch_y.to(self.device, non_blocking=cuda)
            ...
            # Early stopping (in-memory best weights)
        ...
        print(f'Training {status}. Best validation loss: {best_val_loss:.4f}')
        return {'cancelled': cancelled, 'completed_epochs': completed_epochs,
                'best_val_loss': float(best_val_loss)}
