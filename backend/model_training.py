...
def _safe_mape(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """MAPE that skips zero targets to avoid division-by-zero."""
...
class AdvancedModelTrainer:
    ...
    def split_data(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, ...]:
        """Split data into train/validation/test sets (chronological order)."""
    ...
    def cross_validate_model(self, model_type: str, X: np.ndarray, y: np.ndarray,
        ...
        # Use TimeSeriesSplit instead of shuffled KFold to avoid leaking
        # future observations into the training folds.
    ...
    def ensemble_training(self, model_types: List[str], X_train: np.ndarray,
        ...
        ensemble_metrics = {
            ...
            'mape': _safe_mape(y_test, ensemble_pred)
        }
